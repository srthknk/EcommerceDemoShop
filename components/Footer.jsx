            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-8 sm:gap-10 py-8 sm:py-10 border-b border-slate-500/30 text-slate-500">
                    <div className="w-full md:w-auto">
                        ...
                        <p className="max-w-[410px] mt-4 sm:mt-6 text-sm sm:text-base leading-relaxed">Welcome to {storeName}, your ultimate destination for the latest fashion trends and style. From trendy T-shirts and comfortable jeans to elegant kurtas and traditional sarees, we bring you the best in clothing — all in one place.</p>
                        <div className="flex items-center gap-3 mt-4 sm:mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} target="_blank" className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:bg-indigo-600 hover:text-white transition rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                                ...
                                <ul className="space-y-2 sm:space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-start gap-2">
                                            {link.icon && <link.icon className="flex-shrink-0 mt-0.5" />}
